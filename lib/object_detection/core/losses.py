  ...
  def __call__(self,
    ...
    for ind, detection_boxlist in enumerate(decoded_boxlist_list):
      ...
      mined_location_losses.append(
          tf.reduce_mean(tf.gather(location_losses[ind], selected_indices)))
      mined_cls_losses.append(
          tf.reduce_mean(tf.gather(cls_losses[ind], selected_indices)))
    location_loss = tf.reduce_mean(tf.stack(mined_location_losses))
    cls_loss = tf.reduce_mean(tf.stack(mined_cls_losses))
