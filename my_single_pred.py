...
def main():
    ...
    with tf.Session() as sess:
        ...
        restore_model(sess, '/home/yuzhe/PycharmProjects/test_mbmd/model/ssd_mobilenet_video1', variables_to_restore)
        ...
        for i in xrange(epochs):
            for idx, (templates_batch, imgs_batch, gts_batch, labels_batch) in enumerate(otb_loader):
                ...
                print np.sort(cls_loss_b)[-num_match*6:][::-1],'\n'
                # print np.sum(np.sort(cls_loss_b)[-num_match*5:])/num_match

                print cls_loss_b[select1]
                # print np.sum(cls_loss_b[select1])/num_match
                # print 'my calculate cls loss ',np.mean(cls_loss_b[select1])



                ## visualize the accurary of prediction / Then write to img -------------------------------------------------------

                det = detection_dict['detection_boxes'][0][0]  ## y1x1y2x2
                print det
                print iou_y1x1y2x2(det,gt1)

                imgs_ = np.squeeze(imgs_batch)
                draw_mulitbox(Image.fromarray(imgs_),[(det*300).astype(np.int32),(gt1*300).astype(np.int32)],img_path='tmp/{}.jpg'.format(idx))
