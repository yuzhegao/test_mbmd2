...
class MobileTracker(object):
    def __init__(self, init_gt):
        ...
        # checkpoint_dir = '/home/yuzhe/PycharmProjects/test_mbmd2/MBMD_vot_model/model/dump'
    ...
    def test(self,init_img_array,init_gt,image,last_gt,cur_gt):
        ...
        cropped_img, last_gt_norm, win_loc, scale = crop_search_region(cur_ori_img, last_gt, 300,
        ...
        # print detection_box_ori.shape,scores.shape  (100, 4) (1, 100)

        det_search_region = detection_box_ori.copy() ## backup
        ...
        det = det_search_region[max_idx].reshape(4,)


        self.last_gt = detection_box
        # self.target_w = detection_box[3] - detection_box[1]
        # self.target_h = detection_box[2] - detection_box[0]   ## here we can keep the search_region size unchanged
        ...
        draw_mulitbox(cur_ori_img, [box_show.astype(np.int32),gt_show.astype(np.int32)], img_path='../tmp3/{}_1.jpg'.format(self.i))
        cv2.imshow('1', cv2.imread('../tmp3/{}_1.jpg'.format(self.i)))
        cv2.waitKey(1)

        ## show the detection result in crop_search_region
        # box_show1 = np.array([det[1], det[0], det[3], det[2]])
        # draw_mulitbox(cropped_img, [box_show1.astype(np.int32)], img_path='../tmp3/{}_1.jpg'.format(self.i))
        # cv2.imwrite('../tmp4/{}_2.jpg'.format(self.i),cur_img_array)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        region, confidence = tracker.test(init_image, init_gt, image, tracker.last_gt, cur_gt)
