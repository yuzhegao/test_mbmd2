...
class MobileTracker(object):
    def __init__(self, image, region, seq_name):
        init_training = True
        config_file = 'MBMD_vot_model/model/ssd_mobilenet_tracking.config'
        checkpoint_dir = 'model/vid_model_backup'
        ...
        vggMSaver.restore(self.sess, 'MBMD_vot_model/ckpt/VGGM/vggMParams.ckpt')
    ...
    def track(self, image):
        ...
        cur_ori_img_width,cur_ori_img_height = cur_ori_img.size

        search_box1 = detection_box_ori[max_idx]
        search_box1[0] = np.clip(search_box1[0], 0, cur_ori_img_height - 1)
        search_box1[2] = np.clip(search_box1[2], 0, cur_ori_img_height - 1)
        search_box1[1] = np.clip(search_box1[1], 0, cur_ori_img_width - 1)
        search_box1[3] = np.clip(search_box1[3], 0, cur_ori_img_width - 1)  ## y_min x_min y_max x_max
        ...
        if score_max < 0:
            ...
            haha = np.ones_like(search_box[:, 2]) * cur_ori_img_width - 1 - search_box[:, 2]
            search_box[:, 0] = np.minimum(search_box[:, 0], haha)
            haha2 = np.ones_like(search_box[:, 3]) * cur_ori_img_height - 1 - search_box[:, 3]
        ...
        if scores[0, max_idx] < 0.3:  # and score_max < 20.0:
            ...
            search_gt[0] = cur_ori_img_height / 2.0 - (self.last_gt[2] - self.last_gt[0]) / 2.0
            search_gt[2] = cur_ori_img_height / 2.0 + (self.last_gt[2] - self.last_gt[0]) / 2.0
            search_gt[1] = cur_ori_img_width / 2.0 - (self.last_gt[3] - self.last_gt[1]) / 2.0
            search_gt[3] = cur_ori_img_width / 2.0 + (self.last_gt[3] - self.last_gt[1]) / 2.0 ## in the center of img
            ...
            if scores1[0, 0] > 0.8:
                ...
                search_box1[2] = np.clip(search_box1[2], 0, cur_ori_img_height - 1)
                search_box1[1] = np.clip(search_box1[1], 0, cur_ori_img_width - 1)
                search_box1[3] = np.clip(search_box1[3], 0, cur_ori_img_width - 1)
                if (search_box1[0] == search_box1[2]) or (search_box1[1] == search_box1[3]):
                    score_max = -1
                else:
                    search_box1 = [search_box1[1], search_box1[0], search_box1[3] - search_box1[1],
            ...
            ## ====================================================
            if score_max < 0:
                search_gt[0] = cur_ori_img_height / 2.0 - self.first_h / 2.0
                search_gt[2] = cur_ori_img_height / 2.0 + self.first_h / 2.0
                search_gt[1] = cur_ori_img_width / 2.0 - self.first_w / 2.0
                search_gt[3] = cur_ori_img_width / 2.0 + self.first_w / 2.0
            ...
            if score_max < 0:
                search_gt[0] = cur_ori_img_height / 2.0 - self.first_h / 2.0 / 2.0
                search_gt[2] = cur_ori_img_height / 2.0 + self.first_h / 2.0 / 2.0
                search_gt[1] = cur_ori_img_width / 2.0 - self.first_w / 2.0 / 2.0
                search_gt[3] = cur_ori_img_width / 2.0 + self.first_w / 2.0 / 2.0  ## search_gt size reduce ???
            ...
            if score_max < 0:
                ...
                search_gt[2] = cur_ori_img_height / 2.0 + self.first_h / 2.0 * 2.0
                search_gt[1] = cur_ori_img_width / 2.0 - self.first_w / 2.0 * 2.0
                search_gt[3] = cur_ori_img_width / 2.0 + self.first_w / 2.0 * 2.0
        ...
        if scores[0, max_idx] < 0.3:
            ...
            count_research = 0
            isfind = 0
            #print cur_ori_img_width / 2.0 / last_reliable_w, cur_ori_img_height/2.0/last_reliable_h
            while count_research < 500 and (self.startx < cur_ori_img_width + 2 * last_reliable_w - 1) and self.starty < cur_ori_img_height + 2 * last_reliable_h - 1:
                ...
                if self.starty >= cur_ori_img_height + 2 * last_reliable_h - 1 and self.startx < cur_ori_img_width + 2* last_reliable_w-1:
                    self.starty = 0
                    ## here we update the self.startx
                    self.startx = 2 * last_reliable_w + self.startx

            if self.startx >= cur_ori_img_width + 2* last_reliable_w-1:
                self.startx = 0
                self.starty = 0
        ...
        if scores[0, max_idx] > 0.5 and score_max > self.first_score/2.0:
            ...
            #         if neg_examples2[count,0] > cur_ori_img_width-1-neg_examples2[count,2]:
            #             neg_examples2[count,0] = cur_ori_img_width-1-neg_examples2[count,2]
            #         if neg_examples2[count,1] > cur_ori_img_height-1-neg_examples2[count,3]:
            #             neg_examples2[count,1] = cur_ori_img_height-1-neg_examples2[count,3]
            #         count += 1
            #
            #     t+=1
            #
        ...
        if self.last_gt[2] > cur_ori_img_height:
            self.last_gt[2] = cur_ori_img_height - 1
            self.last_gt[0] = cur_ori_img_height - 1 - self.target_h
        if self.last_gt[3] > cur_ori_img_width:
            self.last_gt[3] = cur_ori_img_width - 1
            self.last_gt[1] = cur_ori_img_width - 1 - self.target_w
        ...
        save_img = "model/result_MBMD2/{}/{}.jpg".format(self.seq_name,self.i)
        show_res(image, np.array(self.last_gt, dtype=np.int32), 'MBMD Tracker',
                 score=scores[0,max_idx],score_max=score_max,save_path=save_img)
...
for idx,seq_name in enumerate(seq_list):
    ...
    result_dir = "model/result_MBMD2/{}".format(seq_name)
    ...
    t1 = time.time()
    tracker = MobileTracker(image, selection,seq_name)
    t2 = time.time()
